...
- 확인 결과(출원인 확인, 2026-09-13): 논문에 "모든 트랜지스터" 및 "LVF 데이터 재사용"에 대한 언급은 없다. 내부 트랜지스터 확장은 사내 비공개 논의로만 존재하였고 공개되지 않았으므로 공지 기술이 아니다. 남은 확인 포인트: 민감도 라이브러리의 형식(sensitivity 그룹 여부), 향후 과제(future work)에 내부 TR 확장이 언급되었는지(언급이 있으면 "과제는 알려졌으나 해결 수단은 개시되지 않음" 논리로 대응).
- 조치: 명세서 개정 2.1판에서 이 논문을 가장 근접한 선행기술로 [0005], [0006], [0008], [0015]에 연결하였다. 사내 논의는 명세서에 인용하지 않고, 발명자 확정과 사내 선출원 여부 확인 항목으로 검토 메모에 남겼다.
