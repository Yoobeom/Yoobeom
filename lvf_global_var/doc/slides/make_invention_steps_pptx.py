...
def slide2(prs):
    ...
    box(s, 0.54 + W + 0.35, 1.74, W, 1.98,
        [("동일한 S1(i, q)을 절대값 정규화", {"size": 10.5, "space": 3}),
         ...
         ("부호를 유지하면 오차가 더 작다 (ρ ≥ 0.5 그룹에 한함)", {"size": 10.5, "space": 3}),
         ("C(i) = S1(i) / Σ_j S1(j)", {"size": 11, "mono": True, "space": 6}),
         ("slew × load 격자, 타이밍 값 종류(지연 / 천이)마다 표면으로 저장", {"size": 10, "color": GRAY, "space": 0})],
        fill=TINT, line=NAVY)
    ...
    textbox(s, 0.54, 6.08, 12.26, 0.95,
            [("적층 구조에서는 스위칭 TR이 기여도를 지배한다. 파라미터별로 값이 달라, vth와 u0의 기여도가 0.23 차이난다.",
              {"size": 10.5, "space": 3}),
             ("따라서 파라미터 독립 기여도를 쓰려면 임계값 판정(테이블 평균 |C_vth − C_u0| ≤ 0.1)을 거쳐야 한다.",
              {"size": 10.5, "space": 3}),
             ("같은 아크라도 지연과 출력 천이의 기여도는 다르다 (M1 vth: 지연 0.834, 천이 0.546). 타이밍 값 종류별로 따로 둔다.",
...
def slide3(prs):
    ...
    table(s, 0.54, 4.48, [3.4, 2.9, 3.0, 2.96], [0.52, 0.56, 0.56],
          ...
          [["부호 유지  C(i) = S1(i)/ΣS1(j)", "0.005 %", "0.060 %", "ρ ≥ 0.5 그룹 (구동 그룹 전부)"],
           ["부호 없음  C(i) = |S1(i)|/Σ|S1(j)|", "0.535 %", "0.542 %", "ρ < 0.5 그룹 (비구동, 상쇄)"]])
    textbox(s, 0.54, 6.26, 12.26, 0.6,
            [("모든 TR을 같은 양으로 움직이면 집합값이 그대로 복구되므로, 예측된 TR별 몫은 서로 모순되지 않는다.",
              {"size": 10.5, "space": 4}),
             ("부호 합이 상쇄되는 그룹(ρ = |ΣS1| / Σ|S1| < 0.5)에서는 부호 유지 기여도가 발산한다 (예제: 비구동 33/351, 구동 0/360).",
              {"size": 10.5, "color": GRAY})])
