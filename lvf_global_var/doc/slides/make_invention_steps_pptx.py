...
STEPS = [
    ...
    ("STEP 2", "TR별 민감도 복원",
     ["USM 라이브러리의 집합 민감도", "S_agg(p)"],
     ["S_rec(i, p) = C(i) · S_agg(p)"],
     ["TR별 파라미터 민감도", "전역 변동 예측에 사용"]),
    ("STEP 3", "내부 TR 변동량 확보",
...
def slide1(prs):
    s = slide_head(prs, "TR 기여도 × USM 결합으로 재특성화 없이 변동 반영",
                   "LVF 특성화 결과를 재사용하여 트랜지스터별 민감도를 복원하고, 모든 변동 원인을 하나의 식으로 처리한다.",
    ...
    box(s, X0, yb, 4 * W + 3 * GAP, 1.10,
        [("결합 결과      ΔD_arc = Σ_i Σ_p  C(i ; s,l) · S_agg(p ; s,l) · Δp(i, p)",
         ...
         ("개별 TR 특성화 시뮬레이션 추가 없음. 새로운 변동 원인은 Δp(i, p)만 교체하면 동일 경로로 처리된다.",
          {"size": 11, "space": 0})],
...
def slide2(prs):
    ...
    box(s, 0.54 + W + 0.35, 1.74, W, 1.98,
        [("동일한 S1(i, q)을 절대값 정규화", {"size": 10.5, "space": 3}),
         ...
         ("부호를 유지하면 복원 오차가 더 작다", {"size": 10.5, "space": 3}),
...
# ---------------------------------------------------------------------------
def slide3(prs):
    s = slide_head(prs, "USM에 기여도를 곱하면 TR별로 분해된다",
                   "USM 민감도는 셀 전체를 동시에 섭동한 값이라 TR 식별 정보가 없다. 기여도가 그 정보를 보완한다.",
    ...
    for i, (head, body, mono) in enumerate([
            ("문제", "USM 민감도 S_agg(p)는 셀 전체의 합이다. 트랜지스터별로 나눌 근거가 라이브러리 안에 없다.", None),
            ("해결", "S_rec(i, p) = C(i ; s,l) · S_agg(p ; s,l)", True),
    ...
    textbox(s, 0.54, 4.12, 12.26, 0.3,
            [("복원 정확도 — 그룹 동시 섭동으로 얻은 직접 민감도 대비 (3σ, 지연 대비 %)",
              {"size": 10.5, "bold": True})])
    ...
    textbox(s, 0.54, 6.26, 12.26, 0.6,
            [("정합성 조건을 만족하므로, 동시 변동 시 타이밍 변화량이 개별 TR 변화량의 합과 일치한다.",
              {"size": 10.5, "space": 4}),
...
def slide4(prs):
    ...
    table(s, 5.35, 1.66, [2.05, 2.85, 2.85], [0.52, 0.70, 0.70, 0.86, 0.70],
          ...
          [["보정 대상 TR", "경계 TR만", "경계 + 내부 전체"],
           ["민감도 획득", "TR별 직접 특성화 시뮬레이션", "기여도 × 집합 민감도로 복원"],
...
def slide5(prs):
    ...
    table(s, 0.54, 2.56, [3.3, 5.5, 1.9, 1.56], [0.48, 0.48, 0.48, 0.48, 0.48],
          ...
          [["국부 레이아웃 효과 (LLE)", "레이아웃 추출, 경계 + 내부 TR", "없음", "그대로"],
           ["전역 공정 변동", "공정 코너 정의  ΔVth, ΔU0", "없음", "그대로"],
    ...
    table(s, 0.54, 5.42, [5.6, 3.4, 3.26], [0.48, 0.48, 0.48, 0.48],
          ...
          [["1차 합산 vs 그룹 동시 섭동", "3σ", "차이 최대 0.07 % of delay"],
           ...
           ["logquad 모델 값 오차", "±3σ 복합 코너", "평균 0.3 %, 최대 1.7 %"]])
